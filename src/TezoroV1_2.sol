...
contract TezoroV1_2 is ERC4626, ReentrancyGuard {
    ...
    ///         Paused strategies are included in totalAssets (funds are safe, just operationally
    ///         halted). Excluding them would drop share price and create an arbitrage window.
    ...
    function _rebalance() internal {
        ...
        // Pass 1: Withdrawals -- free up idle before depositing into under-allocated strategies.
        // Without this two-pass approach, strategies earlier in the array cannot receive funds
        // freed by withdrawals from strategies later in the array, requiring multiple rebalance() calls.
        for (uint256 i = 0; i < strategies.length; i++) {
            IStrategy strategy = strategies[i];
            if (pausedStrategies[strategy]) continue;

            uint256 targetBalance = _targetBalance(strategy, total);
        ...
        // Pass 2: Deposits -- now idle includes funds freed from over-allocated strategies.
        for (uint256 i = 0; i < strategies.length; i++) {
            IStrategy strategy = strategies[i];
            if (pausedStrategies[strategy]) continue;
            if (depositFrozenStrategies[strategy]) continue;
    ...
    /// @dev Compute target balance for a strategy, capped by maxAllocationBps.
    function _targetBalance(IStrategy strategy, uint256 total) internal view returns (uint256) {
    ...
    ///      Each strategy's available is reduced by 2 to account for rounding in underlying
