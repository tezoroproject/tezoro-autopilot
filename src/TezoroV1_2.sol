...
contract TezoroV1_2 is ERC4626, ReentrancyGuard {
    ...
    function deposit(
        ...
        // Refresh trackedBalance from live strategy reads BEFORE fee accrual
        // and share pricing. Without this, share price is computed against a
        // cached NAV that the keeper updates only on reconcile(); any yield or
        // loss accrued between reconciles transfers value between user cohorts
        // (early entrants get a stale-priced share of post-reconcile gains;
        // early exiters offload pre-reconcile losses onto stayers). Live
        // refresh closes the timing window the keeper schedule cannot.
    ...
    function mint(
        ...
        // Live NAV refresh — see deposit() for rationale.
        _refreshTrackedBalances();
        _accruePerformanceFee();
    ...
    ///           response.
    ///        2. Full vault migration, if it ever becomes a hard
    ///           requirement, needs a separate primitive (snapshot +
    ///           admin-chosen escrow / new-vault receiver, gated by
    ///           timelock and a share-transfer freeze).
    ...
    ///         Recognizes accrued yield. Permissionless — share-pricing paths
    ///         already refresh trackedBalance live on every user entry/exit, so
    ///         this function is now primarily for off-chain readers (UI, ERC-4626
    ///         view contracts) and for fee accrual on idle vaults. Broken
    ///         strategies are skipped (tracked balance unchanged) so a single
    ///         broken strategy cannot block reconciliation of healthy ones.
