...
contract CompoundV3StrategyV1_2 is IStrategy {
    ...
    function isHealthy() external view override returns (bool) {
        ...
        // liquidity to service exits. Pre-fix the predicate checked only the
        // pause flags, so a fully-utilised Comet (zero base balance, all
        // assets borrowed out) reported healthy and the rebalancer kept
        // routing fresh capital into it; user redeem-paths then surfaced
        // the empty reserve as a withdrawal failure. The exit-side
        // availableLiquidity calculation is unchanged — health gates
        // deposits, withdrawals stay conservative.
