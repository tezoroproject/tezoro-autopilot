...
contract ERC4626MultiStrategyV1_2 is IStrategy, ReentrancyGuard {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            try sv.maxWithdraw(address(this)) returns (uint256 maxW) {
                ...
                try sv.withdraw(toWithdraw, vault, address(this)) {
                    ...
                    // Fallback: some vaults (e.g., YO.xyz) only support redeem,
                    // not withdraw. Use previewWithdraw (withdraw-side quote,
                    // rounds UP) rather than convertToShares (deposit-side,
                    // rounds DOWN). Round-down quoting can deliver less than
                    // `toWithdraw`, or quote zero shares for a non-zero target
                    // when the sub-vault's share price is above 1 — leaving
                    // redeemable liquidity unreachable via the ordinary path.
                    uint256 shares = sv.previewWithdraw(toWithdraw);
                    // Cap against maxRedeem so a previewWithdraw quote larger
                    // than the sub-vault's per-call capacity does not push the
                    // redeem into a hard revert; recover whatever liquidity is
                    // presently available.
    ...
    function balanceOf() external view override returns (uint256 total) {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            try sv.balanceOf(address(this)) returns (uint256 shares) {
                if (shares > 0) {
                    try sv.convertToAssets(shares) returns (uint256 assetsValue) {
                        total += assetsValue;
                    } catch {}
                }
            } catch {}
    ...
    function isHealthy() external view override returns (bool) {
        ...
        // Reading `IERC4626(sv).totalAssets() > 0` measures global vault
        // non-emptiness, which is unrelated to whether THIS strategy's
        // allocations would be safe. Any other LP holding shares would keep
        // that signal positive even when the sub-vault was paused,
    ...
    function freezeSubVaultDeposits(address subVault) external onlyAdmin {
        ...
        // The freeze flag only blocks our own allocate() path; the child's
        // transferFrom into idle balances accumulating from later deposits
        // or other sub-vault redemptions would otherwise remain possible.
        IERC20(asset).forceApprove(subVault, 0);
