...
///      Trust assumption: keeper is trusted to provide correct routerData in swap().
///      The routerData calldata is opaque -- only balance-before/after and slippage
///      checks enforce correctness. A compromised keeper could craft routerData that
///      swaps a different token or amount, limited by the forceApprove on tokenIn.
contract RewardsModuleV1_2 is ReentrancyGuard {
    ...
    event ClaimExecuted(address indexed target, bytes4 indexed selector, bytes returnData);
    event Swapped(
    ...
    function executeClaim(address target, bytes calldata data) external onlyAdminOrKeeper whenNotPaused nonReentrant {
        ...
        emit ClaimExecuted(target, selector, returnData);
    }
