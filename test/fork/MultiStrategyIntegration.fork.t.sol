...
contract ERC4626MultiIntegrationTest is Test {
    ...
    function test_emergencyRecallToIdle() public {
        // Post audit-fix(5)/(34): removeStrategy reverts with
        // StrategyNotFullyRecovered when emergencyWithdraw cannot fully match
        // the live tracked balance. For sub-allocating strategies (Morpho
        // multi, ERC4626 multi) per-market share/asset rounding leaves a few
        // wei behind, so the operator-safe escape is recallToIdle: drain
        // everything that liquidly comes back, freeze deposits, and leave the
...
contract MorphoBlueMultiIntegrationTest is Test {
    ...
    function test_emergencyRecallToIdle() public {
        ...
        // recallToIdle is the correct exit primitive post audit-fix(5)/(34)
        // for sub-allocating strategies.
        vm.prank(alice);
        vault.deposit(DEPOSIT, alice);
        vm.prank(keeper);
        vault.rebalance();
